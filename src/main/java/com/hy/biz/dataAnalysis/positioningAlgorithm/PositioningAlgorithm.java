 ...
 * 故障精准定位：采用双端定位算法
 */
package com.hy.biz.dataAnalysis.positioningAlgorithm;

import com.hy.biz.dataAnalysis.dto.FaultPositioningAnalysisResult;
...
public class PositioningAlgorithm {
    ...
    public PositioningAlgorithm(HyConfigProperty hyConfigProperty) {
        this.hyConfigProperty = hyConfigProperty;
    }

    public Optional<FaultPositioningAnalysisResult> analyze(Set<FaultWave> faultWaves) {
        ...
        Optional<DevicePairDistance> minPair = computeFaultDist(phaseSortedWaveMap);  // 计算故障距离

        FaultPositioningAnalysisResult result = null;
        if (minPair.isPresent()) {
            result = new FaultPositioningAnalysisResult();
            result.setDistanceBetweenStations(minPair.get().getL2() + minPair.get().getL3());
            result.setNearestPoleId(minPair.get().getNearestPoleId());
        }
    ...
    private Optional<DevicePairDistance> computeFaultDist(Map<Integer, List<FaultWave>> phaseSortedWaveMap) {
        ...
        // fixme 在所有的L2、L3中找到采集的两台设备中按照距离间隔最近的这两台计算出来的L2、L3
    ...
    private Optional<DevicePairDistance> analyzeFaultWaveByPhase(List<FaultWave> phaseSortedWaves) {
        ...
        DevicePairDistance result = new DevicePairDistance();
        result.setL2(L2);
        result.setL3(L3);
        ...
        result.setNearestPoleId(nearestPoleId);
    ...
    private void addValidatedSpeedToList(List<Double> waveSpeeds, List<FaultWave> waves) {
        for (int i = 0; i < waves.size() - 2; i += 2) {
            ...
            double waveSpeed = distance / deltaT; // Calculate wave speed in m/us
